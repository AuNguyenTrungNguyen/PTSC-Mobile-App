...
import { GetInstrumentSupportRegisterListAPI, UpdateInstrumentSupportPaintingAPI } from '../../../apis/eit/EITAPI';

import SelectPopup from '../../../components/SelectPopup';
import { ListSelectData, ListLoadingData, ListEmptyData } from '../../../components/HelperUI';
import LoadingRefresh from '../../../components/LoadingRefresh';

const InstrumentSupportPaintingListScreen = ({ route, navigation }) => {
  ...
  async function getEITItems(filterResult = result) {
    ...
    GetInstrumentSupportRegisterListAPI(projectCode, facility, drawingNo, location, name, filterResult)
      .then(res => {
  ...
  const submitToServer = async () => {
    ...
    setIsUploading(true);
    UpdateInstrumentSupportPaintingAPI(listUpdate)
      .then(res => {
        if (res.Success) {
          setEITUpdateItems([]);
          callAPI(getEITItems);
  ...
  const renderItem = ({ _, item }) => {
    ...
    const disableREQ = false;
    const disableFirstCoat = false;
    const disableSecondCoat = false;
    const disableThirdCoat = false;
    const disableFinalCoat = false;
    const disableAccepted = false;
    ...
    return (
      <View style={styles.box}>
        ...
        <View style={styles.row}>
          ...
          <View style={styles.cellOne}>
            <Text style={styles.textData}>{Formater.formatDateData(item.REQForPaintingDate)}</Text>
          </View>
          <View style={styles.cellOneRow}>
            <Text>{'Sent:'}</Text>
            <CheckBox
              value={!!item.REQForPaintingDate}
              onValueChange={newValue => _onChangeCheckbox(item.RowIndex, 'REQForPaintingDate', newValue)}
              style={styles.checkBox}
              boxType='square'
              disabled={disableREQ}
              onCheckColor={OPP_COLOR}
              onFillColor={disableREQ ? DISABLE_COLOR : BASE_COLOR}
              onTintColor={disableREQ ? DISABLE_COLOR : BASE_COLOR}
        ...
        <View style={styles.row}>
          ...
          <View style={styles.cellOne}>
            <Text style={styles.textData}>{Formater.formatDateData(item.PaintingFirstCoat)}</Text>
          </View>
          <View style={styles.cellOneRow}>
            <Text>{'Sent:'}</Text>
            <CheckBox
              value={!!item.PaintingFirstCoat}
              onValueChange={newValue => _onChangeCheckbox(item.RowIndex, 'PaintingFirstCoat', newValue)}
              style={styles.checkBox}
              boxType='square'
              disabled={disableFirstCoat}
              onCheckColor={OPP_COLOR}
              onFillColor={disableFirstCoat ? DISABLE_COLOR : BASE_COLOR}
              onTintColor={disableFirstCoat ? DISABLE_COLOR : BASE_COLOR}
              tintColors={{ true: BASE_COLOR, false: DISABLE_COLOR }}
              animationDuration={0.2}
              onAnimationType='flat'
            />
        ...
        <View style={styles.row}>
          ...
          <View style={styles.cellOne}>
            <Text style={styles.textData}>{Formater.formatDateData(item.PaintingSecondCoat)}</Text>
          </View>
          <View style={styles.cellOneRow}>
            <Text>{'Sent:'}</Text>
            <CheckBox
              value={!!item.PaintingSecondCoat}
              onValueChange={newValue => _onChangeCheckbox(item.RowIndex, 'PaintingSecondCoat', newValue)}
              style={styles.checkBox}
              boxType='square'
              disabled={disableSecondCoat}
              onCheckColor={OPP_COLOR}
              onFillColor={disableSecondCoat ? DISABLE_COLOR : BASE_COLOR}
              onTintColor={disableSecondCoat ? DISABLE_COLOR : BASE_COLOR}
              tintColors={{ true: BASE_COLOR, false: DISABLE_COLOR }}
              animationDuration={0.2}
              onAnimationType='flat'
            />
        ...
        <View style={styles.row}>
          ...
          <View style={styles.cellOne}>
            <Text style={styles.textData}>{Formater.formatDateData(item.PaintingThirdCoat)}</Text>
          </View>
          <View style={styles.cellOneRow}>
            <Text>{'Sent:'}</Text>
            <CheckBox
              value={!!item.PaintingThirdCoat}
              onValueChange={newValue => _onChangeCheckbox(item.RowIndex, 'PaintingThirdCoat', newValue)}
              style={styles.checkBox}
              boxType='square'
              disabled={disableThirdCoat}
              onCheckColor={OPP_COLOR}
              onFillColor={disableThirdCoat ? DISABLE_COLOR : BASE_COLOR}
              onTintColor={disableThirdCoat ? DISABLE_COLOR : BASE_COLOR}
              tintColors={{ true: BASE_COLOR, false: DISABLE_COLOR }}
              animationDuration={0.2}
              onAnimationType='flat'
            />
        ...
        <View style={styles.row}>
          <View style={styles.cellOne}>
            <Text>{'Final Coat'}:</Text>
          </View>
          <View style={styles.cellOne}>
            <Text style={styles.textData}>{Formater.formatDateData(item.PaintingFinalCoat)}</Text>
          </View>
          <View style={styles.cellOneRow}>
            <Text>{'Sent:'}</Text>
            <CheckBox
              value={!!item.PaintingFinalCoat}
              onValueChange={newValue => _onChangeCheckbox(item.RowIndex, 'PaintingFinalCoat', newValue)}
              style={styles.checkBox}
              boxType='square'
              disabled={disableFinalCoat}
              onCheckColor={OPP_COLOR}
              onFillColor={disableFinalCoat ? DISABLE_COLOR : BASE_COLOR}
              onTintColor={disableFinalCoat ? DISABLE_COLOR : BASE_COLOR}
              tintColors={{ true: BASE_COLOR, false: DISABLE_COLOR }}
              animationDuration={0.2}
              onAnimationType='flat'
            />
        ...
        <View style={styles.row}>
          ...
          <View style={styles.cellOne}>
            <Text style={styles.textData}>{Formater.formatDateData(item.PaintingAcceptedDate)}</Text>
          </View>
          <View style={styles.cellOneRow}>
            <Text>{'Sent:'}</Text>
            <CheckBox
              value={!!item.PaintingAcceptedDate}
              onValueChange={newValue => _onChangeCheckbox(item.RowIndex, 'PaintingAcceptedDate', newValue)}
              style={styles.checkBox}
              boxType='square'
              disabled={disableAccepted}
              onCheckColor={OPP_COLOR}
              onFillColor={disableAccepted ? DISABLE_COLOR : BASE_COLOR}
              onTintColor={disableAccepted ? DISABLE_COLOR : BASE_COLOR}
              tintColors={{ true: BASE_COLOR, false: DISABLE_COLOR }}
              animationDuration={0.2}
              onAnimationType='flat'
            />
...
export default InstrumentSupportPaintingListScreen;