...
import { GetElectricalGlandControlDetailAPI, UpdateElectricalGlandControlDetailAPI } from '../../../apis/eit/EITAPI';
...
const ElectricalGlandControlDetailScreen = ({ route, navigation }) => {
  ...
  async function getCableDetail() {
    GetElectricalGlandControlDetailAPI(rowIndex)
      .then(res => {
  ...
  const updateCableDetail = async () => {
    setIsUploading(true);
    UpdateElectricalGlandControlDetailAPI(glandDetail, columnChange)
      .then(res => {
        setIsUploading(false);
        if (res.Success) {
...
export default ElectricalGlandControlDetailScreen;