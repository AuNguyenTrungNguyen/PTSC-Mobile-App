...
import { GetInstrumentEquipmentControlListAPI, UpdateInstrumentEquipmentControlListAPI } from '../../../apis/eit/EITAPI';

import SelectPopup from '../../../components/SelectPopup';
import { ListSelectData, ListLoadingData, ListEmptyData } from '../../../components/HelperUI';
import LoadingRefresh from '../../../components/LoadingRefresh';

const InstrumentEquipmentControlListScreen = ({ route, navigation }) => {
  ...
  async function getEITItems() {
    ...
    GetInstrumentEquipmentControlListAPI(projectCode, facility, drawingNo, location, name)
      .then(res => {
  ...
  const submitToServer = async () => {
    ...
    setIsUploading(true);
    UpdateInstrumentEquipmentControlListAPI(listUpdate)
      .then(res => {
        if (res.success) {
          setEITUpdateItems([]);
  ...
  const renderItem = ({ _, item }) => {
    const status = item.InstallCompleteDate ? 1 : 0;
    ...
    return (
      <View style={styles.box}>
        ...
        <View style={styles.row}>
          ...
          <View style={styles.cellTwo}>
            <Text style={textStyle}>{Formater.formatEmptyData(item.LayoutDrawing)}</Text>
          </View>
        </View>
        <View style={styles.row}>
          <View style={styles.cellOne}>
        ...
        <View style={styles.row}>
          ...
          <View style={styles.cellTwo}>
            <Text style={textStyle}>{Formater.formatEmptyData(item.TagNo)}</Text>
          </View>
        </View>
        <View style={styles.row}>
          <View style={styles.cellOne}>
          ...
          <View style={styles.cellTwo}>
            <View style={styles.cellTwoRow}>
              <Text>Install:</Text>
              <CheckBox
                value={!!item.InstallCompleteDate}
                onValueChange={newValue => _onChangeCheckbox(item.RowIndex, 'InstallCompleteDate', newValue)}
          ...
          <View style={styles.container}>
            {
              isShowDescription.show
                ...
                (<View style={styles.headerContainer}>
                  ...
                  <View style={styles.rowInfoAction}>
                    <Text style={styles.infoTitleAction}>TagNo:</Text>
                    <View style={styles.inputContainer}>
                      <TextInput
                        style={styles.inputText}
                        value={name}
...
export default InstrumentEquipmentControlListScreen;