...
import { GetStructureDrawingDataTypeAPI, GetStructureDrawingAPI } from '../../../apis/general/GeneralAPI';
...
const OpenStructuralDrawingScreen = ({ route, navigation }) => {
  ...
  const [sourceFilter, setSourceFilter] = useState(FILTER_DEFAULT);
  const getSourceList = async () => {
    GetStructureDrawingDataTypeAPI(projectCode)
      .then(res => {
        if (res.Success) {
  ...
  const _onChangeSourceCode = data => {
    if (data !== sourceFilter) {
      setSourceFilter(data);
    }
    setIsVisibleSource(false);
  };
  const _onClearSourceCode = () => {
    if (sourceFilter !== FILTER_DEFAULT) {
      setSourceFilter(FILTER_DEFAULT);
    }
    setIsVisibleSource(false);
  };
          ...
          <View style={styles.cellThree}>
            {
              item.WebLink
                ?
                <TouchableOpacity onPress={() => Helper.openDrawingPDF(navigation, item.WebLink, 'Structural')}>
  ...
  const RenderList = () => {
    {
      if (isSearching) {
        ...
        return <ListSelectData title={'Please select Type / Drawing'} />
      } else {
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
          ...
          <View style={styles.container}>
            {
              isShowDescription.show
                ...
                (<View style={styles.headerContainer}>
                  ...
                  <View style={styles.rowInfoAction}>
                    <Text style={styles.infoTitleAction}>DrawingType:</Text>
                    <TouchableOpacity style={styles.selectInput} onPress={() => setIsVisibleSource(true)}>
                      <Text style={styles.buttonTitleDark}>{sourceFilter}</Text>
                    </TouchableOpacity>
                  </View>
                  <View style={styles.rowInfoAction}>
      ...
      <SelectPopup
        ...
        data={sourceList}
        onCancel={() => setIsVisibleSource(false)}
        onClear={_onClearSourceCode}
        onChangeItem={_onChangeSourceCode} />
    </SafeAreaView>
  );
};
...
export default OpenStructuralDrawingScreen;