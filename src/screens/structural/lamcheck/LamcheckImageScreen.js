...
import {
  GetLamCheckTodoImageAPI,
  DeleteLamCheckTodoImageAPI,
  EditLamCheckTodoImageAPI
} from '../../../apis/structural/LamCheckAPI';
...
const LamcheckImageScreen = ({ route }) => {
  ...
  const { projectCode, userLogin, dataCode, rowIndex, drawingNo, jointNo } = route.params;
  ...
  const getImage = async () => {
    ...
    GetLamCheckTodoImageAPI(rowIndex, token)
      .then(res => {
        if (res.success) {
  ...
  const _onPressUploadImage = async () => {
    ...
    addFilesToBody()
      .then(res => {
        ...
        RNFetchBlob.fetch(
          'POST',
          Port_Server + '/api/structural/LamCheck/UploadLamCheckTodoImage',
          {
  ...
  const deleteImage = async id => {
    let token = await Helper.getData('TOKEN');
    DeleteLamCheckTodoImageAPI(id, token)
      .then(res => {
  ...
  const _onPressUpdateImage = async () => {
    let token = await Helper.getData('TOKEN');
    EditLamCheckTodoImageAPI(pictureId, pictureNote, token)
      .then(res => {
        Toast.show(res.Message.toString(), Toast.SHORT);
        if (res.success) {
...
export default LamcheckImageScreen;