...
import { GetValveProgressListAPI } from '../../../apis/valve/ValveAPI';

import { ListSelectData, ListLoadingData, ListEmptyData } from '../../../components/HelperUI';
import LoadingRefresh from '../../../components/LoadingRefresh';
import SelectPopup from '../../../components/SelectPopup';

const ValveProgressListScreen = ({ route, navigation }) => {
  ...
  const [valveList, setValveList] = useState(null);
  ...
  //-- Search Action
  const _onPressSearchValve = () => {
    Keyboard.dismiss();
    callAPI(getLineNoData);
  };
  async function getLineNoData({ facility = facilityCode } = {}) {
    facility = (!facility || facility === FACILITY_CODE_DEFAULT) ? '' : facility;
    GetValveProgressListAPI(projectCode, facility, lineNo)
      .then(res => {
        if (res.Success && res.Data) {
          setValveList(res.Data);
          setIsLoading(false);
          setIsError(false);
    ...
    return (
      <TouchableOpacity style={styles.box} onPress={() => _onPressDetail(item)}>
        ...
        <View style={styles.row}>
          <View style={styles.cellThree}>
            <Text>LineNo:   </Text>
            <Text style={styles.textData}>{Formater.formatEmptyData(item.Line_No)}</Text>
          </View>
        </View>
        <View style={styles.row}>
          <View style={styles.cellThree}>
  ...
  const RenderList = () => {
    {
      if (isSearching) {
        ...
        return <ListSelectData title={'Please select Facility / LineNo'} />
      } else {
    ...
    <SafeAreaView style={styles.safeArea}>
      {
        isLoading || isError
          ?
          <LoadingRefresh isLoading={isLoading} isError={isError} _onPressRefresh={() => callAPI(getLineNoData)} />
          :
          <View style={styles.container}>
            {
              isShowDescription.show
                ...
                (<View style={styles.headerContainer}>
                  ...
                  <View style={styles.rowInfoAction}>
                    <Text style={styles.infoTitleAction}>LineNo:</Text>
                    <View style={styles.inputContainer}>
                      <TextInput
                        style={styles.inputText}
                        value={lineNo}
                        onChangeText={_onChangeLineNo}
                        underlineColorAndroid='transparent'
                      />
                      {
                        lineNo == ''
                          ? null
                          : <Icon name='times-circle' onPress={() => _onChangeLineNo('')} style={styles.inputIcon} />
                      }
                    </View>
                  </View>
                  <View style={styles.rowInfoAction}>
...
export default ValveProgressListScreen;