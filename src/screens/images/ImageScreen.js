...
import { GetFlangeJointProgressImageAPI, UploadFlangeJointProgressImageAPI, DeleteImageAPI, EditImageAPI } from '../../apis/images/ImageAPI';
...
const ImageScreen = ({ route }) => {
  ...
  const { userLogin, projectCode, facilityCode, drawingNo, sheet, jointNo, rowIndex, code, role, type } = route.params;
  ...
  const getImage = async () => {
    if (type) {
      if (type === Constant.IMAGE_TYPE_FLANGE) {
        GetFlangeJointProgressImageAPI(rowIndex)
          .then(res => {
            if (res.Success) {
  ...
  const _onPressUploadImage = async () => {
    ...
    addFilesToBody()
      .then(res => {
        ...
        UploadFlangeJointProgressImageAPI(body)
          .then(res => {
            if (res.Success) {
